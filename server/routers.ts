  ...
  userEvidences: router({
    ...
    generateAllPDF: protectedProcedure
      .mutation(async ({ ctx }) => {
        ...
        // توليد باركود QR للمعلم
        const QRCode = await import('qrcode');
        const teacherId = ctx.user.id;
        const qrCodeDataURL = await QRCode.toDataURL(`teacher:${teacherId}`, {
          width: 300,
          margin: 1,
          color: {
            dark: '#000000',
        ...
        // 0. صفحة الغلاف مع باركود QR
        const currentYear = '1446';
        const coverHTML = introPages.generateCoverPage({
          teacherName: profile.teacherName || ctx.user.name || '',
          year: `${currentYear}هـ`,
          qrCodeDataURL: qrCodeDataURL,
          coverTheme: profile.preferredCoverTheme || 'theme1',
        });
        introPdfBuffers.push(await htmlToPdf(coverHTML));
