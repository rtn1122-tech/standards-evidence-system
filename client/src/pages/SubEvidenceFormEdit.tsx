...
export default function SubEvidenceFormEdit() {
  ...
  // Fetch subTemplate data
  const { data: subTemplate } = trpc.evidenceSubTemplates.getById.useQuery(
    { id: evidenceDetail?.subTemplateId! },
    { enabled: !!evidenceDetail?.subTemplateId }
  );
...
تحسين رضا أولياء الأمور والمجتمع عن مستوى التنظيم داخل المدرسة.`);
  ...
  useEffect(() => {
    if (evidenceDetail) {
      ...
      if (customFields.additionalFields) {
        try {
          const additionalFields = JSON.parse(customFields.additionalFields);
          setDynamicFields(additionalFields);
        } catch (e) {
  ...
  const handlePreview = async () => {
    if (!evidenceDetail?.subTemplateId || !profile) {
    ...
    previewMutation.mutate({
      subTemplateId: evidenceDetail.subTemplateId,
