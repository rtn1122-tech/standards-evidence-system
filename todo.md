...
## تعبئة محتوى الشواهد الـ 11
- [x] تعبئة شاهد "الحضور والانصراف" (ID 101):
  - [x] المقدمة المختصرة (الصفحة الأولى)
  - [x] المربعات الـ 6 (الصفحة الثانية)
- [ ] تعبئة باقي الشواهد (102-111) - حسب النصوص المرسلة
- [ ] اختبار العرض في المتصفح
- [ ] حفظ checkpoint

## ضبط المحاذاة ونهاية الأسطر
- [x] ضبط المحاذاة إلى اليمين في جميع textarea - إضافة dir="rtl" و className="text-right"
- [x] ضبط نهاية الأسطر - leading-relaxed لتباعد الأسطر
- [x] إصلاح أخطاء TypeScript - دمج className المكررة
- [ ] اختبار العرض في المتصفح

## تطبيق الثيم الرسمي الجديد
- [x] تحليل تصميم الثيم الرسمي 1 من الصورة المرسلة
- [x] حذف الثيم القديم نهائياً من generatePDF.ts
- [x] تطبيق الثيم الجديد:
  - [x] Header: شعار وزارة التعليم + نص (المملكة + الوزارة + الإدارة + المدرسة) + باركود + رؤية 2030 + خط أخضر + مثلثات
  - [ ] Footer: دائرة خضراء + خطوط مائلة + مربعات التوقيع
- [x] إصلاح ترتيب المربعات الخضراء:
  - [x] في SubEvidenceFormNew.tsx: المربع الأخضر على اليمين قبل الاسم
  - [x] في generatePDF.ts: المربع الأخضر على اليمين قبل الاسم
- [ ] اختبار PDF وحفظ checkpoint

## استخدام الثيم كخلفية في PDF
- [x] تحويل PDF الثيم إلى صورة
- [x] حفظ صورة الثيم في المشروع
- [x] إعادة كتابة generatePDF.ts لاستخدام صورة الخلفية
- [x] تثبيت مواقع: الباركود (بين النص والرؤية)
- [x] وضع محتوى الشاهد فوق الخلفية
- [ ] اختبار PDF وحفظ checkpoint

## إصلاح مواقع المحتوى في PDF
- [ ] حذف النص المكرر في الصفحة الأولى (يظهر في المربع الكبير الأسفل فقط)
- [ ] إصلاح مواقع اسم المعلم والمدير في الصفحة الثانية (في المربعات الخضراء أسفل الصفحة)
- [ ] إصلاح الخانات الـ 6 الفارغة في الصفحة الثانية
- [ ] حذف جميع النصوص/التصاميم المضافة (استخدام صورة الثيم فقط)
- [ ] اختبار PDF وحفظ checkpoint

## إصلاحات إضافية من ملاحظات المستخدم
- [x] إزاحة الباركود إلى اليسار (left: 65mm)
- [x] إضافة إدارة التعليم واسم المدرسة تحت "وزارة التعليم" في الوسط
- [x] ضبط موقع مربعات التوقيع (bottom: 18mm) لتكون في المربعات الخضراء
- [ ] اختبار PDF وحفظ checkpoint

## إضافة النص الافتراضي والخط Traditional Arabic
- [x] إضافة النص الافتراضي للخانات الـ 6 في SubEvidenceFormNew.tsx
- [x] تطبيق direction: rtl و text-align: right في PDF
- [x] تغيير الخط إلى Traditional Arabic لإدارة التعليم واسم المدرسة
- [ ] اختبار PDF وحفظ checkpoint

## إصلاح زر تعديل الشاهد
- [x] فحص زر التعديل في صفحة "شواهدي"
- [x] إصلاح المشكلة - إنشاء SubEvidenceFormEdit.tsx
- [x] إضافة update procedure في server/routers.ts
- [x] إضافة updateEvidenceDetail function في server/db.ts
- [x] إضافة route في App.tsx
- [ ] اختبار التعديل
- [ ] حفظ checkpoint

## الإصلاحات النهائية من ملاحظات المستخدم
- [x] إصلاح نهاية الأسطر في الصفحة الأولى (line-height: 1.8, text-align: right, direction: rtl)
- [x] النص في الصفحة الثانية صحيح (نفس النص المرسل)
- [x] ضبط خط وحجم إدارة التعليم واسم المدرسة (font-size: 14px, font-weight: bold, color: #000)
- [x] إنزال مربعات التوقيع (bottom: 12mm)
- [ ] اختبار PDF النهائي
- [ ] حفظ checkpoint

## طلبات جديدة - إضافة حقول ديناميكية ومحاذاة النص

- [ ] إضافة زر (+) لإضافة حقول ديناميكية غير محدودة (أكثر من 8) في جميع الشواهد
- [ ] إضافة زر حذف (×) لكل حقل إضافي
- [ ] محاذاة النص في الوسط (text-center) لجميع المربعات في النموذج
- [ ] محاذاة النص في الوسط في PDF
- [ ] تنسيق المربعات في وسط الصفحة في PDF
- [ ] حفظ الحقول الديناميكية في قاعدة البيانات
- [ ] عرض الحقول الديناميكية في PDF

## حذف المحتوى وإعادة الترتيب
- [x] حذف جميع الشواهد المحفوظة من evidenceDetails
- [x] حذف المحتوى الافتراضي من evidenceSubTemplates (description, sections, images)
- [x] الاحتفاظ بالقوالب الـ 11 (IDs: 101-111)
- [x] الاحتفاظ بجميع التعديلات (الحقول الديناميكية، المحاذاة، إلخ)
- [x] التحقق من النتائج
- [ ] حفظ checkpoint

## توحيد تنسيق الحقول الإضافية
- [x] توحيد تنسيق الحقول الإضافية (زر +) في SubEvidenceFormNew.tsx
- [x] توحيد تنسيق الحقول الإضافية (زر +) في SubEvidenceFormEdit.tsx
- [x] التأكد من تطابق الحجم والخط والتباعد مع الحقول الأصلية
- [x] اختبار التنسيق
- [x] حفظ checkpoint

## تعميم خاصية تعديل عناوين الحقول على جميع الشواهد
- [x] تحديث SubEvidenceFormNew.tsx لتطبيق أيقونة القلم على جميع الحقول الثمانية
- [x] تحديث SubEvidenceFormEdit.tsx لتطبيق أيقونة القلم على جميع الحقول الثمانية
- [x] تحديث generatePDF.ts لدعم العناوين المخصصة لجميع الشواهد (ليس فقط 102)
- [x] إضافة state للعناوين المخصصة (field5-8 labels) لجميع الشواهد
- [x] اختبار التعديل على شاهد غير 102
- [x] حفظ checkpoint

## حذف الشواهد الفرعية وإبقاء شاهد واحد للتطوير
- [x] تحديد الشاهد الذي سيتم الاحتفاظ به (الشاهد 101 - الحضور والانصراف)
- [x] حذف 10 شواهد فرعية (102-111) من جدول evidenceSubTemplates
- [x] الواجهة تعرض الشاهد الواحد تلقائياً (لا حاجة لتعديل)
- [x] اختبار إنشاء وتعديل وعرض الشاهد المتبقي
- [x] التأكد من عمل جميع الميزات (تعديل العناوين، الحقول الديناميكية، PDF)
- [x] حفظ checkpoint

## إضافة ميزة معاينة PDF قبل الحفظ
- [x] إضافة tRPC procedure لتوليد PDF مؤقت (generatePreviewPDF)
- [x] إضافة زر "معاينة PDF" في SubEvidenceFormNew
- [x] إضافة Dialog/Modal لعرض PDF باستخدام iframe
- [x] تطبيق نفس الميزة في SubEvidenceFormEdit
- [x] إضافة loading state أثناء توليد المعاينة
- [x] اختبار المعاينة مع جميع أنواع البيانات
- [ ] حفظ checkpoint
