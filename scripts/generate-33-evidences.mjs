...
async function generateEvidenceContent(evidenceInfo) {
  const { title, standardName, subTitle } = evidenceInfo;
  
  const prompt = `أنت خبير تربوي متخصص في إعداد شواهد المعلمين للترقية المهنية في السعودية.

**المطلوب:** إنشاء محتوى احترافي لشاهد بعنوان "${title}" ضمن المعيار "${standardName}".
...
- **العام الدراسي:** 1447هـ (وليس 1446 أو 1445)
...
1. **الوصف** (12-15 سطر):
   ...
   - **line-height:** استخدم أسلوب يناسب line-height: 2
...
async function main() {
  console.log('🚀 توليد 33 شاهد...\n');

  const listPath = path.join(__dirname, '..', 'data', 'selected-33-list.json');
  const evidenceList = JSON.parse(fs.readFileSync(listPath, 'utf-8'));

  const results = [];

  for (let i = 0; i < evidenceList.length; i++) {
    const evidence = evidenceList[i];
    console.log(`⏳ [${i + 1}/33] توليد: ${evidence.title}...`);
    
    try {
      const generated = await generateEvidenceContent(evidence);
      results.push(generated);
      console.log(`✅ [${i + 1}/33] تم بنجاح!`);
    } catch (error) {
      console.error(`❌ [${i + 1}/33] خطأ:`, error.message);
      results.push({ ...evidence, error: error.message });
    }
  ...
  const outputPath = path.join(__dirname, '..', 'data', 'generated-33-evidences.json');
  fs.writeFileSync(outputPath, JSON.stringify(results, null, 2), 'utf-8');

  console.log(`\n✅ تم توليد ${results.length} شاهد بنجاح!`);
